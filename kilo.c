...
void enableRawMode() {
    if (tcgetattr(STDIN_FILENO, &E.orig_termios) == -1) die("tcgettar");
...
void editorDrowRows(struct abuf *ab) {
    int y;
        ...
        if (filerow >= E.numrows) {
            ...
            for (j = 0; j < len; j++) {
                if (iscntrl(c[j])) {
                    ...
                    if (curren_color != -1) {
                        char buf[16];
                        int clen = snprintf(buf, sizeof(buf), "\x1b[%dm", curren_color);
...
void editorDrawStatusBar(struct abuf *ab) {
    ...
    int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
        E.filename ? E.filename : "[Nu Name]", E.numrows,
        E.dirty ? "(modified)" : "");
